...
export const status: RequestHandler = async (req, res) => {
    ...
    try {
        ...
        const tempoRestante = Number(resource[0].EXECUT * Number(decrypted(sanitize(String(req.cookies["qtdLibMax"])))) * 1000 - (Number(new Date().getTime() - decrypted(String(sanitize(req.cookies['startSetupTime'])))))) || 0
