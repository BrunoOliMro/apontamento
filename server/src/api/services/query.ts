...
export const selectQuery = async (chonsenOption: number, values?: any) => {
    ...
    const codes = {
        ...
        // 11: `UPDATE PCP_PROGRAMACAO_PRODUCAO SET QTDE_APONTADA = QTDE_APONTADA + ${values.valorTotalApontado}, QTD_REFUGO = COALESCE(QTD_REFUGO, 0) + ${values.badFeed}, QTDE_LIB = ${values.released}, QTD_FALTANTE = ${values.missingFeed}, QTD_BOAS = COALESCE(QTD_BOAS, 0) + ${values.goodFeed}, QTD_RETRABALHADA = COALESCE(QTD_RETRABALHADA, 0) + ${values.reworkFeed} WHERE 1 = 1 AND NUMERO_ODF = ${values.NUMERO_ODF} AND CAST (LTRIM(NUMERO_OPERACAO) AS INT) = ${values.NUMERO_OPERACAO} AND CODIGO_MAQUINA = '${values.CODIGO_MAQUINA}'`,
        // 12: `UPDATE PCP_PROGRAMACAO_PRODUCAO SET QTDE_APONTADA = ${values.valorApontado}, QTD_BOAS = QTD_BOAS - ${values.goodFeed}, QTD_FALTANTE = ${values.faltante}, QTDE_LIB = ${values.qtdLib}, QTD_REFUGO = QTD_REFUGO - ${values.badFeed}, QTD_ESTORNADA = COALESCE(QTD_ESTORNADA, 0 ) + ${values.valorTotal} WHERE 1 = 1 AND NUMERO_ODF = '${values.NUMERO_ODF}' AND CAST (LTRIM(NUMERO_OPERACAO) AS INT) = '${Number(values.NUMERO_OPERACAO)}' AND CODIGO_MAQUINA = '${values.CODIGO_MAQUINA}'`,
        ...
        // 15: `UPDATE PCP_PROGRAMACAO_PRODUCAO SET TEMPO_APTO_TOTAL = GETDATE() WHERE 1 = 1 AND NUMERO_ODF = ${values.NUMERO_ODF} AND CAST (LTRIM(NUMERO_OPERACAO) AS INT) = ${values.NUMERO_OPERACAO} AND CODIGO_MAQUINA = '${values.CODIGO_MAQUINA}'`,
        ...
        20: `SELECT [CODIGO], [IMAGEM] FROM VIEW_APTO_FERRAMENTAL WHERE 1 = 1 AND IMAGEM IS NOT NULL AND CODIGO = '${values.CODIGO_PECA}'`,
    ...
    try {
        ...
        // const result = await select(chonsenOption, values, )
        // if (result.length > 0) {
        //     return { message: message(1), data: result }
        // } else {
        //     return { message: message(1), data: result }
        // }
    } catch (error) {
